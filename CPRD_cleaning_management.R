...
###### Step 8. Removing test codes that are not actually tests
###### Step 9. Restricting date range to pre-covid (ending 2019)
###### Step 10. Exploring the data -  test counts
###### Step 11. Extraction of each test (for test specific analyses)
###### Step 12. Extracting all tests in 2019 (for practice variation analysis)
...
# First, I replaced MCS unspecified and Sample MCS with Microbiology NOS as these were deemed sufficiently similar to group
...
# Next, I want to see which panels tend to co-exist (occur on the same day for the same patient) with unspecified codes to try and work out what the unspecified test code may be
...
# This code will correctly count the co-occurrences of panels other than "Urine MCS" on the same day for the same patient and arrange them by frequency. 

### Sample analysis NOS 

# Filter the dataset to include only "Sample analysis NOS" rows
sampleanalysisnos_data <- merged_df_panel_2 %>%
  filter(panel == "Sample analysis NOS")

# Get a list of unique patients and observation dates for "Sample analysis NOS"
...
# Most commonly occurs with Urine MCS, Microbiology NOS, Urinalysis 


### Microbiology NOS

# Filter the dataset to include only "Microbiology NOS" rows
microbiologynos_data <- merged_df_panel_2 %>%
  filter(panel == "Microbiology NOS")

# Get a list of unique patients and observation dates for "Microbiology NOS"
...
# Most commonly occurs with Urine MCS, Urinalysis, FBC, Sample analysis NOS
# Other microbiology tests in order - Stool MCS, Genital MCS, Sputum MCS, Fungal MCS, Wound/skin MCS, ENT MCS, Enteric virus screen, Stool OCP
...
# Get a list of unique patients and observation dates for "Scan NOS"
...
# Most commonly occurs with FBC, LFT, Urea and electrolytes, Microbiology NOS, TFT



### Adenovirus

# Filter the dataset to include only "Adenovirus test" rows
adeno_data <- merged_df_panel_2 %>%
  filter(panel == "Adenovirus test")

# Get a list of unique patients and observation dates for "Adenovirus test"
...
# Most commonly occurs with Microbiology NOS, Enteric virus screen, Stool OCP, Stool MCS
...
# Most commonly occurs with Microbiology NOS, Not diagnostic test, Stool OCP, Stool MCS, Ecoli 0157, Enteric virus screen
...
# Get a list of unique patients and observation dates for "Urine test NOS"
...
# Most commonly occurs with Urinalysis, Urine MCS, Microbiology NOS
...
# Most commonly occurs with Microbiology NOS, Enteric virus screen, FBC, Stool OCP
...
# Most commonly occurs with FBC, LFT, Urea and electrolytes, Microbiology NOS, Bone profile, EBV test
...
# Filter the dataset to include only "Haematology NOS" rows
haemnos_data <- merged_df_panel %>%
  filter(panel == "Haematology NOS")

# Get a list of unique patients and observation dates for "Haematology NOS"
...
# Occurs with FBC, LFT, Urea and electrolytes, Bone profile
...
# Occurs with FBC, LFT, Urea and electrolytes, Bone profile
# Other biochemistry tests include (in order): TFT, CRP, Iron studies, Glucose, Allergen-specific IgE, Lipids, Urine biochemistry
...
# Exists alongside spirometry and Not diagnostic test 
...
# Get a list of unique patients and observation dates for "US NOS"
...
# Occurs with FBC, LFT, Urea and electrolytes, Urine MCS -- difficult to discern which ultrasound it is likely to be based on these panels
...
# Get a list of unique patients and observation dates for "XR NOS"
...
# No clear winner - FBC, LFT, Urea and electrolytes, XR finger, XR wrist, XR foot
...
# Get a list of unique patients and observation dates for "CT NOS"
...
# No clear winner -> results are FBC, urea and electrolytes, MRI NOS, LFT
...
# Get a list of unique patients and observation dates for "MRI NOS"
...
# No clear winner --> FBC, EEG, Urea and electrolytes, LFT, ECG 




######################### Step 6. Examining, identifying and replacing likely duplicates######################### 
...
# Check individual patient id

dummy <- subset(merged_df_panel_5, merged_df_panel_5$patid=="6078841621419")   # examining id 6078841621419 which had 2 of same test on one day by creating dummy dataframe 
...
# Test to see if there are both Microscopy and Stool OCP occurring together, answer should be 0 
...
######################### Step 8. Remove tests that are not actually diagnostic tests ######################### 
...
######################### Step 10. Most common test numbers overall  ######################### 
...
######################### Step 12. Preparing dataframe including tests only in 2019 ######################### 
